...
function Signayo({ isVisible, setIsVisible }) {
  const hiddenStyle = { display: "none" };

  const closeModal = () => {
    setIsVisible(false);
  };
  const videoId = "2sV1Azsvypo";
    ...
    <div className={style.modal} style={!isVisible ? hiddenStyle : {}}>
      <div className={style.innerContainer}>
        ...
        <p className={style.heading}>
          <span> Signayo:</span> How to translate Filipino Movie Clips to
        ...
        <div className={style.btnContaier}>
          <button onClick={closeModal}>
            <Link className={style.link} href="/speech">
              Let's Go{" "}
            </Link>
          </button>
        </div>
